    ...
    def __call__(self, target):
        ...
        for obj in target.iter("object"):
            ...
            for j, kpt in enumerate(kpts):
                cur_kpt = int(float(kpnts.find(kpt).text)) - 1
                # keypoints.append(cur_kpt)
            ...
            # keypoints.append(label_idx)
            # kep = np.empty((0,7))
            # kep = np.vstack((kep, keypoints))  # [x1, y1, v1, x2, y2, v2, x3, y3, v3, label_ind]
...
class VOCDetection(CacheDataset):
    ...
    def load_anno_from_ids(self, index):
        ...
        #process/resize keypoints

        sift = cv2.SIFT_create()
        img = self.load_image(index)
        resized_img = self.load_resized_img(index)
        keypoints = cv2.KeyPoint_convert(res[:, 4:10])
        keypoints_resized = sift.compute(resized_img, keypoints)[0]

        # Compute the descriptors for the original image
        _, descriptors = sift.detectAndCompute(img, None)

        # Re-compute the descriptors
        keypoints_resized, descriptors_resized = sift.compute(resized_img, keypoints_resized)

        # Match the keypoints
        bf = cv2.BFMatcher(cv2.NORM_L2, crossCheck=True)
        matches = bf.match(descriptors, descriptors_resized)
